...
The directed graph $ \mathcal{G} = (\mathcal{V}, \mathcal{E}, \mathcal{A}) $ is taken in consideration to describe the information exchange between agents. Here $ \mathcal{A} = [a_{ij}] \in \mathbb{R}^{N \times N} $ represents the adjacency matrix, where  $ \mathcal{V}=[v_1, v_2,\dots,v_N] $ is the set of vertices, and $ \mathcal{E} = [(v_j,v_i)|v_i \in \mathcal{V} ] \subseteq \mathcal{V} \times \mathcal{V}$ is the set of edges. Moreover, $ \mathcal{N}_i = {j \in \mathcal{V} |(i,j) \in \mathcal{E} ,i \neq j} $ denotes the set of neighbors of agent $ i $. No self-loop can be allowed in this article, which means $(i, i) \in \mathcal{E}$ for any $ i \in \mathcal{V} $, $ a_{ii}=0$. Furthermore, the degree matrix is denoted by $ K $, where $ K = diag(k_1,\dots,k_N)$. If $ k_i > 0$, agent $ i $ can directly obtain the information from the leader. The Laplacian matrix $ L $ can be defined as $ L = (\mathcal{D}-\mathcal{A})$, here $\mathcal{D} = diag({d_1,\dots,d_N})$, and $d_i=\sum_{j=1}^{N} a_{i j}$ denotes the in-degree matrix. The graph is strongly connected if the path exists between every pair of vertices.
...
% \noindent 
where $u_i(k) \in \mathbb{R}$ and $y_i(k) \in \mathbb{R}$ are the system input signal and output signal at instant $ k $, respectively. $f_i(\cdot)$ signifies an unknown nonlinear function.

Assumption 1: The partial derivative of \( f_i(\cdot) \) with respect to \( u_i(k) \) is continuous.

Assumption 2: The system (\ref{model 1}) is generalized Lipschitz, meaning that if \( \Delta u_i(k) = u_i(k) - u_i(k - 1) \neq 0 \), then \( | \Delta y_i(k + 1) | \leq b |\Delta u_i(k)| \) holds for any \( k \). A positive constant \( b \) exits such as \( \Delta y_i(k + 1) = y_i(k + 1) - y_i(k) \).
