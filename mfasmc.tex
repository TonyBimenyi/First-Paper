...
\begin{abstract}
    In this paper, the distributed data-driven sliding mode control approach is proposed to investigate the consensus problem of nonlinear multi-agent systems. Firstly, the equivalent data model is constructed using the compact-form dynamic linearization (CFDL) technique. Secondly, the novel sliding surface is employed to enhance trajectory convergence rate and ensuring that the distributed measurement error remains bounded. Subsequently, a distributed model-free adaptive sliding mode controller scheme is then presented to realize that consensus tracking can be achieved using I/O data of each agent. Finally, the effectiveness of the proposed scheme is verified through multi-DC motors.
\end{abstract}
...
This paper focuses on multi-agents systems composed of one leader and $ N $ followers, whose dynamics are heterogeneous and nonlinear. Discribing the information exchange between agents, a signed graph $ \mathcal{G} = (\mathcal{V}, \mathcal{E}, \mathcal{A}) $ is taken in consideration, where $ \mathcal{A} = [a_{ij}] \in \mathbb{R}^{N \times N} $ represents a weighted adjacency matrix with -1, 0 and 1 elements, and $ \mathcal{V}=[v_1, v_2,\dots,v_N] $ is the set of vertices, $ \mathcal{E} = [(v_j,v_i)|v_i \in \mathcal{V} ] \subseteq \mathcal{V} \times \mathcal{V}$ is the set of edges.  Moreover, let $ \mathcal{N}_i = {j \in \mathcal{V} |(i,j) \in \mathcal{E}} $ indicates the index set of neighbors of $ v_i $ where $ a_{i,j} \neq 0 $, if $ (v_j, v_i) \in \mathcal{E} $; $ a_{ij} = 0 $. We assume that no self-loop can be allowed in this article, meaning that, $(i, i) \in \mathcal{E}$ for any $ i \in \mathcal{V} $, $ a_{ii}=0$. Furthermore, a degree matrix is denoted by $ K $, where $ K = diag(k_1,\dots,k_N)$. If $ k_i > 0$, the agent $ i $ can directly obtain the information from the leader. The Laplacian matrix $ L $ of $ \mathcal{G} $ can be given as $ L = (\mathcal{D}-\mathcal{A})$, where $\mathcal{D} = diag({d_i})^N_{i=1}$, and $d_i=\sum_{j \in N_i} |a_{i j}|$ denotes the in-degree matrix. If there exists a path between any pair of two vertices, the graph is strongly connected.

\subsection{Problem Formulation}

Consider the nonlinear multi-agent systems composed of $ N $ agents, the following dynamic of each agent is given by:
...
% \noindent 
where $u_i(k) \in \mathbb{R}$ and $y_i(k) \in \mathbb{R}$ are the system input signal and output signal at instant $ k $, respectively, and $f_i(\cdot)$ is an unknown nonlinear function.

Assumption 1: The partial derivative of the nonlinear function \( f_i(\cdot) \) with respect to \( u_i(k) \) is continuous.

Assumption 2: The system model (\ref{model 1}) is generalized Lipschitz, meaning that if \( \Delta u_i(k) = u_i(k) - u_i(k - 1) \neq 0 \), then \( | \Delta y_i(k + 1) | \leq b |\Delta u_i(k)| \) holds for any \( k \), where \( \Delta y_i(k + 1) = y_i(k + 1) - y_i(k) \) and \( b \) is a positive constant.



Remark 1: The practical applicability of the aforementioned assumptions to nonlinear systems has been discussed in \cite{2} and assumption 1 establishes a general condition for controller design while, assumption 2 implies that the rate of the control input is bounded. This constraint ensures that, from an energetic perspective, finite changes in control input energy correspond to bounded changes in output energy rates, a crucial consideration for system stability and performance.
...
Lemma 1 \cite{8}: Consider follower agents with dynamic (\ref{model 1}) satisfying above two assumptions. If $ ||\Delta u_i(k) || \neq 0 $ holds, then system (\ref{model 1}) can be transformed into a compact form dynamic linearization(CFDL) data model as follows:
...
Remark 2: By application of CFDL technique, the unknown nonlinear system dynamic (\ref{model 1}) are now transformed into a time-varying incremental form data (\ref{model 2}) in every operation point with a bounded pseudo-partial derivative. 
 
Let denote the following distributed measurement output of \(\xi_i(k)\) for $N$ agents as follow:
