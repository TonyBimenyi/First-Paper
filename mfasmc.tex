...
\begin{abstract}
    In this paper, the model-free adaptive sliding mode control (MFASMC) approach is proposed to investigate the consensus problem of nonlinear multi-agent systems. Firstly, the equivalent data model is constructed using the compact-form dynamic linearization (CFDL) technique. Secondly, the novel slding surface is employed to enhance trajectory convergence rate and ensuring that the distributed measurement error remains bounded. Subsequently, the model-free adaptive sliding mode control scheme is presented to realize that consensus tracking can be achieved under fixed topology using I/O data of each agent. Finally, simulation verification and experimental validation on multi-DC motors system are conducted to demonstrate effectiveness of the proposed design method.
\end{abstract}
