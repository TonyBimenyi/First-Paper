...
Considering ($ \ref{model eq:sms}) $, with the assistance of reaching law $ S_i(k+1) = 0 $, the equivalent control law is then designed as
...
The controller contains equivalent control law and switching control law, which is set as
...
Taking in considaration both(\ref{model eq:equivalent_control}), (\ref{model eq:u}) and (\ref{model eq:reaching_law})  , the controller is summarized as follows:
...
Theorem 1: For the MASs (\ref{model 1}) that satifies Assumptions 1 and 2, assuming $ |\xi_{i,0}| < \min\{a_{i,0}, b_{i,0}\} \rho_{i,0} $ are satisfied, the controller parameters establish $ 0<h_0<h_i(k)<\frac{\omega C_0}{2 \sqrt{\sigma}} <1 $, then using the PPD update law (\ref{model eq:ppd_parameter}) can ensure the boundedness of $ \hat{\phi}_i(k) $.

Proof: The proof is devided in two parts.
...
Denote that the term $ \frac{\eta \Delta u_i(k-1)^2}{\mu + \Delta u_i(k-1)^2} $ is monotonically increasing with respect to $| \Delta u_i(k)^2 |$, and its minimun value is $ \frac{\eta \epsilon^2}{\mu + \epsilon^2} $, therefore there must be a constant $ q_1 $ to satisfy inequality $ 0 < \eta \leq 1 $ and $ u > 0$
...
Part ii: In this part, mathematical induction will be utilized to prove that $ \xi_i(k)$ is remain bounded.
...
\[
\begin{array}{c}
\text{Agent 1}: \quad y_1(k+1) = \frac{m u_1(k)}{0.1 T}\\
\text{Agent 2}: \quad y_2(k+1) = \frac{m u_2(k)}{0.1 T } \\
\text{Agent 3}: \quad y_3(k+1) = \frac{m u_3(k)}{0.3 T } \\
\text{Agent 4}: \quad y_4(k+1) = \frac{m u_4(k)}{0.3 T } 
\end{array}
\]
...
Exemple 1: The expression for \( y_d(k) \) is:
