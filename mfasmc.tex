...
In order to verify the proposed consensus tracking control methodology, the experimental verification was conducted using a multi-DC motors system as illustrated in Fig.\ref{fig:hardware_con}. The system consists three DC motors equipped with Hall encoders and reduction gears, a STM32F407 main control chip, two motor drive modules, and a LCD display module. The microcontroller unit (MCU) is the STM32F407ZGT6, for the high-resolution pulse width modulation (PWM) output generation for precise control of motor speed, the timer module was utilized.

In addtion, C language is used to write the code of the controller in STM32CubeIDE and STM32CubeMX for pin configuration; the main purpose of the experiment is to make three motors track the reference trajectory as: 
