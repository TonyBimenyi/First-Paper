...
\begin{abstract}
    This paper proposes the distributed data-driven sliding mode control approach to address the consensus problem of nonlinear multi-agent systems. Firstly, the equivalent data model for each agent is constructed using the compact-form dynamic linearization (CFDL) technique. Secondly, utilizing the input-output data of each agent, the novel sliding surface which ensures the boundedness of distributed error through the stability-guaranteeing mechanism is employed. Finally, the effectiveness of the proposed control approach is verified through experiments on multi-DC motors.
\end{abstract}
