...
% \noindent 
where $u_i(k) \in \mathbb{R}$ and $y_i(k) \in \mathbb{R}$ are the system input and output signals at instant $ k $, respectively. $f_i(\cdot)$ signifies an unknown nonlinear function.
...
Remark 1\cite{2}: Assumption 1 provides a general condition for controller design, while assumption 2 ensures that the bounded changes in control input result in the bounded changes in output energy rates, supporting system stability and performance. 
