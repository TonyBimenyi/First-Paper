...
\section*{Keywords}
Model-free adaptive sliding mode control, Multi-DC motors speed control, nonlinear multi-agent system, data-driven control.
...
To design the SMC for the system, the sliding mode surface is first defined as
...
\begin{align}
    ...
    \xi_i(k+1) =\ & \xi_i(k) - \bigg(\sum_{j \in N_i} a_{ij} + d_i \bigg) \phi_i(k) \Delta u_i(k) + \sum_{j \in N_i} a_{ij} \Delta y_j(k) \nonumber \\
    &  + d_i \Delta y_d(k+1)
\end{align}
...
Considering ($ \ref{model eq:sms}) $, with the assistance of reaching law $ S_i(k+1) = 0 $, the equivalent control law is designed as
...
The controller consists of an equivalent control law and switching control law, which is set as
...
where, the switching control law $ \Delta u_{i,\text{SM}}^s(k) $ is presented as
...
Taking into considaration both(\ref{model eq:equivalent_control}), (\ref{model eq:u}) and (\ref{model eq:reaching_law}), the controller is summarized as follows:

\begin{align}
    \label{model eq:sm_controller}
    u_{i,\text{SM}}(k) =\ & u_{i,\text{SM}}(k-1) +  \frac{\omega \hat{\phi}_i(k)}{\sigma + \hat{\phi}_i(k)^2} \bigg( \frac{\xi_i(k)+ \sum_{j \in N_i}a_{ij} \Delta y_j(k) + d_i \Delta y_d(k+1)}{\sum_{j \in N_i}a_{ij}+d_i} \nonumber \\
    &\quad - \frac{\xi_i(k)}{\alpha(\displaystyle \sum_{j \in N_i}a_{ij}+d_i)} + \tau_s sign(S_i(k) \bigg)
\end{align}
    


Subsequently, the final MFASMC input  is designed:
...
where the parameter \(\Gamma\) is a gain factor.

\subsection{Stability Analysis}

Theorem 1: For the MASs (\ref{model 1}) satisfying assumptions 1 and 2, assuming the controller parameters establish $ 0<h_0<h_i(k)<\frac{\omega C_0}{2 \sqrt{\sigma}} <1 $, then using the PPD update law (\ref{model eq:ppd_parameter}) ensures the boundedness of $ \hat{\phi}_i(k) $.

Proof: The proof is devided into two parts.

Part i: Set $ \tilde{\phi}_i(k) = \hat{\phi}_i(k)-\phi_i(k) $, based on PPD estimation algorithm ($ \ref{model eq:ppd_parameter} $), we can obtain: 

\begin{align}
    \label{model:phi_tilde_update}
    \tilde{\phi}_i(k) =\ & \tilde{\phi}_i(k-1) + \frac{\eta \Delta u_i(k-1)}{\mu + \Delta u_i(k-1)^2} ( \phi_i(k-1) \Delta u_i(k-1) - \hat{\phi}_i(k-1) \Delta u_i(k-1)) \nonumber \\
    =\,\ & \tilde{\phi}_i(k-1) + \frac{\eta \Delta u_i(k-1)}{\mu + \Delta u_i(k-1)^2} ( \phi_i(k-1) - \hat{\phi}_i(k-1)) - \phi_i(k) + \phi_i(k-1) \nonumber \\
    =\,\ & \bigg( 1 - \frac{\eta \Delta u_i(k-1)^2}{\mu + \Delta u_i(k-1)^2} \bigg) \tilde{\phi}_i(k-1) - \Delta \phi_i(k) 
\end{align}

Denote that the term $ \frac{\eta \Delta u_i(k-1)^2}{\mu + \Delta u_i(k-1)^2} $ is monotonically increasing with respect to $| \Delta u_i(k)^2 |$, and its minimum value is $ \frac{\eta \epsilon^2}{\mu + \epsilon^2} $. Therefore, there must be a constant $ q_1 $ satisfying the inequalities $ 0 < \eta \leq 1 $ and $ u > 0$
...
Because of $ |\phi_i(k)| < \bar{b} $, and $ |\Delta \phi_i(k)| < 2 \bar{b} $ then the equation ($ \ref{model:phi_tilde_update} $) is written as:


\begin{align}
    \label{model:absolute}
    |\tilde{\phi}_i(k)| \leq\ & q_1|\tilde{\phi}_i(k-1)| + 2 \bar{b}  \nonumber \\
    \leq\,\ & q_1^2|\tilde{\phi}_i(k-2)| + 2 q_1\bar{b} + 2\bar{b} \nonumber \\
    \vdots \nonumber \\
    \leq\,\ & q_1^{k-1}|\tilde{\phi}_i(1)| + \frac{2 \bar{b}}{1-q_1}(1-q_1^{k-1}) 
\end{align}   

which implies $ \tilde{\phi}_i(k) $ is bounded. Since the boundedness of $ \phi_i(k) $ is guaranteed by Lemma 1. 
    
Part ii: In this part, mathematical induction will be utilized to prove that $ \xi_i(k)$ remains bounded.

Substituting ($ \ref{model eq:ppd_parameter} $) into ($ \ref{model eq:xi_next} $), then the expression for \(\xi_i(k+1)\) is formulated as follows:

\begin{align}
    \label{model eq:xi_next_proof}
    \xi_i(k+1) =\ & \xi_i(k) + \sum_{j \in N_i}a_{ij} \Delta y_j(k) + d_i \Delta y_d(k+1) - \frac{\omega \phi_i(k) \hat{\phi}_i(k)}{\sigma + \hat{\phi}_i(k)^2} \bigg( (1-\frac{1}{2})\xi_i(k)+ \sum_{j \in N_i}a_{ij} \Delta y_j(k) + d_i \Delta y_d(k+1)\bigg) \nonumber \\
    &\quad + \bigg( \sum_{j \in N_i}a_{ij} +d_i \bigg) \tau_s sign(Si(k)) \nonumber \\
    =\,\ & \bigg( 1 - \frac{\omega \phi_i(k) \hat{\phi}_i(k)}{\sigma + \hat{\phi}_i(k)^2} (1-\frac{1}{2})  \bigg) \xi_i(k) + \bigg( 1 - \frac{\omega \phi_i(k) \hat{\phi}_i(k)}{\sigma + \hat{\phi}_i(k)^2}  \bigg) \bigg ( \sum_{j \in N_i}a_{ij} \Delta y_j(k) + d_i \Delta y_d(k+1) \bigg ) \nonumber \\
    &\quad - \frac{\omega \phi_i(k) \hat{\phi}_i(k)}{\sigma + \hat{\phi}_i(k)^2}  \bigg ( \sum_{j \in N_i}a_{ij} + d_i \bigg ) \tau_s sign(S_i(k))
\end{align}

By applying the fundamentals of mathematical induction, then assuming $ 0<h_0<h_i(k)<\frac{\omega C_0}{2\sqrt{\sigma}}<1 $, where $ \phi_i(k) < C_0 $ and $ |g_i(k)|<g_0 $, the inequality is obtained by taking the absolute value of each term of ($ \ref{model eq:xi_next_proof} $).

\begin{align}
    \label{model:absolute2}
    |\xi_i(k+1)| \leq\ & |1-h_i(k)(1-\frac{1}{2})| |\xi_i(k)| + |g_i(k)|  \nonumber \\
    \leq\,\ & |1-h_i(k)(1-\frac{1}{2})| |\xi_i(k)| + g_0(k) \nonumber \\
    \vdots \nonumber \\
    \leq\,\ & 1-h_0(1-\frac{1}{2})^k |\xi_i(k)| + \frac{g_0(1-(1-h_0(1-\frac{1}{2}))^2)}{h_0(1-\frac{1}{2})} 
\end{align}   
...
\begin{equation}
    \label{model:lim_fin}
    \lim_{k \to \infty} \xi_i(k+1) = \frac{g_0}{h_0(1-\frac{1}{2})} = \frac{\alpha g_0}{(\alpha-1)h_0}
\end{equation}

The proof is completed.


Remark 3: Theorem 1 guarantees the boundedness of  $ \hat{\phi}_i $ and the distributed measurement error for the MASs described by ($\ref{model 1} $) under assumptions 1 and 2. The proof employs the PPD update law ($ \ref{model eq:ppd_parameter} $) to ensure that $ \tilde{\phi}_i(k) $ remains bounded, as demonstrated through the application of the factor $ q_1<1 $. The mathematical induction confirms that $ \xi_i(k) $ is bounded. Thus, this result shows the effectiveness of the proposed controller in achieving stable performance for the MASs.
...
Example 1: The expression for \( y_d(k) \) is:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1 \textwidth]{tracking_inv.png}
...
Fig. \ref{fig:tracking_inv} demonstrates that all agents successfully track the time-invariable reference trajectory. 

As shown in Fig. \ref{fig:error_inv} and \ref{fig:error_var}, the distributed errors for all agents are relatively small and converge to zero over time. However, the individual agents exhibit varying levels of tracking error, suggesting that their unique dynamics or initial conditions may influence the performance.

\begin{figure}[H]
    \centering
    \includegraphics[width=1 \textwidth]{error_inv.png}
    \caption{Distributed error of all agents under the time-invarying reference trajectory.}
    \label{fig:error_inv} % Label for referencing the figure
\end{figure}


Example 2: The expression for \( y_d(k) \) is:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{tracking_var.png}
