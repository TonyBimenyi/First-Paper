...
\begin{abstract}
    This paper proposes the distributed data-driven sliding mode control approach to investigate the consensus problem of nonlinear multi-agent systems. Firstly, the equivalent data model is constructed using the compact-form dynamic linearization (CFDL) technique. Secondly, the novel sliding surface is employed to enhance and ensuring that the distributed measurement error remains bounded. Subsequently, a distributed model-free adaptive sliding mode controller scheme is then given to realize that consensus tracking can be achieved using I/O data of each agent. Finally, the effectiveness of the proposed scheme is verified through multi-DC motors.
\end{abstract}
