...
\begin{align}
    ...
    \xi_i(k+1) =\ & \xi_i(k) - \left(\sum_{j \in N_i} a_{ij} + d_i \right) \phi_i(k) \Delta u_i(k) + \sum_{j \in N_i} a_{ij} \Delta y_j(k) \nonumber \\
    &  + d_i \Delta y_d(k+1)
\end{align}
...
\begin{equation}
    ...
    \Delta u_{i,\text{SM}}^{eq} = \frac{\omega \hat{\phi}_i(k)}{\sigma + \hat{\phi}_i(k)^2}\left(\frac{\xi_i(k)+ \sum_{j \in N_i}a_{ij} \Delta y_j(k) + d_i \Delta y_d(k+1)}{\displaystyle \sum_{j \in N_i}a_{ij}+d_i} - \frac{\xi_i(k)}{\alpha(\displaystyle \sum_{j \in N_i}a_{ij}+d_i)}
    \right)
\end{equation}
...
Taking in considaration both(\ref{model eq:equivalent_control}), (\ref{model eq:u}) and (\ref{model eq:reaching_law})  , the controller is summarized as

\begin{equation}
    \label{model eq:sm_controller}
    u_{i,\text{SM}}(k) = u_{i,\text{SM}}(k-1) +  \frac{\omega \hat{\phi}_i(k)}{\sigma + \hat{\phi}_i(k)^2}\left(\frac{\alpha(\displaystyle \sum_{j \in N_i}a_{ij} y_j(k) + d_i y_d(k+1))-\xi_i(k)}{\alpha(\displaystyle \sum_{j \in N_i}a_{ij}+d_i)} - y_i(k) +\tau_s sign(S_i(k))
    \right)
\end{equation}


Therefore, the final MFASMC input  is designed:
...
Theorem 1: For the MASs (\ref{model 1}) that satifies Assumptions 1 and 2, assuming $ |\xi_{i,0}| < \min\{a_{i,0}, b_{i,0}\} \rho_{i,0} $ are satisfied, the utilizing the PPD update law (\ref{model eq:ppd_parameter}) can ensure the boundedness of $ \hat{\phi}_i(k) $.
