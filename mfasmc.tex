...
\textbf{Example 1:} In this example, the reference trajectory is a time-invariant signal \( y_d(k) = 0.6 \). The initial parameters are chosen as \( u_i(1) = 0 \), \( y_i(1) = 0 \), and \( \hat{\phi}_i(1) = 1 \) for all agents in this simulation. The control gains are set as \( \Gamma_{1} = \Gamma_{3} = 0.45 \) and \( \Gamma_{2} = \Gamma_{4} = 0.15 \). Other parameters are \( \tau_s = 10^{-5} \), \( m = 600 \), \( r_T = 1024 \), \( \eta = 1 \), \( \mu = 0.005 \), \( \rho = 7.5 \), \( \lambda = 350 \), \( \omega = 10 \), \( \sigma = 95 \), \( \alpha = 15 \), and \( \epsilon = 10^{-5} \).
