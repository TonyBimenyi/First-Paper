...
Due to the capabilities and coordination efficiency, distributed multi-agent systems (MASs) excel in applications such as sensor networks, transportation, agriculture monitoring, and indutrial process. This has attracted significant attention to MAS \cite{1,2,3}. Over the past few years, one of the main challenges for collaborative control \cite{4,5,6} is to handle consensus problem. In particular, it enables all agents to converge on the same reference trajectory despite varying initial values, enhancing system robustness. However, traditional model-based control strategies are not suitable for complex systems that are difficult to model accurately. 
...
Remark 2: The CFDL technique requires no prior knowledge about the system dynamic model, making it highly suitable for distributed multi-agent systems. In addition, such methods excel in handling complex systems where modeling is impractical.   Moreover, the dynamic behavior of time-varying PPD may be highly complex, which is challenging to verify. Thus, data-driven control method is employed to adaptively study the dynamic behavior, thereby ensuring effective control performance.
...
Consider the following PPD criterion function for the parameter woth unknown dynamics in (\ref{model 2}):
\begin{equation}
    \label{model 4}
...
\] where $ m $  denotes the total number of time steps. In comparison to the existing approach \cite{1}, the developed strategy demonstrates significant improvement in accuracy, reducing the average MSE across agents by a factor of approximately 1.34. This notable reduction confirms the effectiveness of the proposed technique.
...
measurements for two control strategies are detailed in TABLE \ref{tab:mse_comparison2}. Relative to the
conventional approach \cite{1}, the proposed scheme reduces in average MSE by approximately 1.24.

\begin{table}[h]
    \centering
    \caption{Mean square error comparison (Example 2)}
    \label{tab:mse_comparison2}
    \resizebox{0.45\linewidth}{!}{ % adjust the scale factor as needed
    \begin{tabular}{lcc}
    \toprule
    $ \text{MSE}_i(k) $ & Method \cite{1} & Proposed scheme \\
