...
\begin{abstract}
    This paper proposes the distributed data-driven sliding mode control approach to address the consensus problem of nonlinear multi-agent systems. Firstly, the equivalent data model for each agent is constructed using the compact-form dynamic linearization (CFDL) technique. Secondly, the novel sliding surface is employed ensuring that the distributed measurement error is bounded through an appropriate stability-guaranteeing mechanism. Subsequently, to achieve consensus tracking, a distributed model-free adaptive sliding mode controller scheme is  developed. This approach relies solely on input-output data of each agent. Finally, the effectiveness of the proposed control strategy is verified through experiments on multi-DC motors.
\end{abstract}
