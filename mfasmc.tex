...
\begin{abstract}
    This paper proposes the distributed data-driven sliding mode control approach to address the consensus problem in the network of nonlinear multi-agent systems. Firstly, the equivalent data model for each agent is constructed using the compact-form dynamic linearization (CFDL) technique. Secondly, the novel sliding surface is employed to enhance performance and ensure that the distributed measurement error remains bounded by incorporating the positive constant. Subsequently, to achieve consensus tracking, a distributed model-free adaptive sliding mode controller scheme is then developed. This approach relies solely on input-output data of each agent, eliminating the need for explicit system models. Finally, the effectiveness of the proposed control scheme is verified through multi-DC motors.
\end{abstract}
...
Assumption 2: The system (\ref{model 1}) is generalized Lipschitz, meaning that the positive constant \( b \) exists such that if \( \Delta u_i(k) = u_i(k) - u_i(k - 1) \neq 0 \) then \( | \Delta y_i(k + 1) | \leq b |\Delta u_i(k)| \) holds for any \( k \), where \( \Delta y_i(k + 1) = y_i(k + 1) - y_i(k) \).
...
Remark 1:The assumptions above are general and aligned with reality. Assumption 1 specifies the constraints of the controller. Assumption 2 ensures that variations in the input rate are limited, thereby restricting the change rate in the output, which is satisfied in real industrial systems.

Lemma 1\cite{8}: Consider agents with dynamics described by (\ref{model 1}) satisfying above two assumptions. If $ | \Delta u_i(k) | \neq 0 $ holds, then the system (\ref{model 1}) can be transformed into the compact form dynamic linearization (CFDL) data model as follows:
...
The physical verification was conducted using multi-DC motors system. The system is composed with 3 DC-motors with a STM32F407ZET6 board and connected through 2 motor drivers. The reference trajectory is :



The following figures show the output results of the system.
