...
The directed graph $ \mathcal{G} = (\mathcal{V}, \mathcal{E}, \mathcal{A}) $ is taken in consideration to describe the information exchange between agents. Here $ \mathcal{A} = [a_{ij}] \in \mathbb{R}^{N \times N} $ represents the adjacency matrix  $ \mathcal{V}=[v_1, v_2,\dots,v_N] $ is the set of vertices, $ \mathcal{E} = [(v_j,v_i)|v_i \in \mathcal{V} ] \subseteq \mathcal{V} \times \mathcal{V}$ defines the set of edges.  Moreover, $ \mathcal{N}_i = {j \in \mathcal{V} |(i,j) \in \mathcal{E} ,i \neq j} $ indicates the set of neighbors of agent $ i $.Assume that no self-loop can be allowed in this article, $(i, i) \in \mathcal{E}$ for any $ i \in \mathcal{V} $, $ a_{ii}=0$. Furthermore, the degree matrix is denoted by $ K $, where $ K = diag(k_1,\dots,k_N)$. If $ k_i > 0$, the agent $ i $ can directly obtain the information from the leader. The Laplacian matrix $ L $ can be given as $ L = (\mathcal{D}-\mathcal{A})$, here $\mathcal{D} = diag({d_1,\dots,d_N})$, and $d_i=\sum_{j=1}^{N} a_{i j}$ denotes the in-degree matrix. If there exists a path between any pair of two vertices, the graph is strongly connected.
